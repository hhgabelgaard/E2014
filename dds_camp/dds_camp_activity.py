...
class dds_camp_activity_activity(osv.osv):
    ...
    _columns = {
        ...
        'audience': fields.selection([('par', 'Participants'),
                                      ('staff', 'ITS'),
                                      ('all', 'All')], 'Audience'),
        'act_ins_ids': fields.one2many('dds_camp.activity.instanse',
                                       'activity_id', 'Instanses'),
    }
...
class dds_camp_activity_instanse(osv.osv):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if record.activity_id:
                dummy, act_name = act_obj.name_get(
                    cr, uid, [record.activity_id.id], context)[0]
            if record.period_id:
                dummy, per_name = per_obj.name_get(
                    cr, uid, [record.period_id.id], context)[0]
            if record.seats_hard and context.get('limit_check', False):
                limit_txt = ' (Only %d seats left)' % (record.seats_available)
            if record.name:
                display_name = record.name + ' ' + \
                    act_name + ' - ' + per_name + limit_txt
            else:
    ...
    def _get_seats(self, cr, uid, ids, fields, args, context=None):
        """Get reserved, available, reserved but unconfirmed and used seats for
        each event tickets.
        @return: Dictionary of function field values.
        """
        res = dict([(id, {}) for id in ids])
        for actins in self.browse(cr, uid, ids, context=context):
            res[actins.id]['seats_reserved'] = sum(
                ticket.seats for ticket in actins.ticket_ids if ticket.state == "open")
            res[actins.id]['seats_used'] = sum(
                ticket.seats for ticket in actins.ticket_ids if ticket.state == "done")
    ...
    def _search_seats(self, cr, uid, obj, name, args, context=None):
        print args
        ids = []
        for ai in self.browse(cr, uid, self.search(cr, uid, [], context=context),
                              context=context):
    ...
    _columns = {
        ...
        'seats_reserved': fields.function(_get_seats, string='Reserved Seats',
                                          type='integer',
                                          multi='seats_reserved'),
        'seats_available': fields.function(_get_seats,
                                           string='Available Seats',
                                           type='integer',
                                           multi='seats_reserved',
                                           fnct_search=_search_seats),
        'seats_used': fields.function(_get_seats,
                                      string='Number of Participations',
                                      type='integer',
                                      multi='seats_reserved'),
        'activity_id': fields.many2one('dds_camp.activity.activity',
                                       'Activity'),
        'period_id': fields.many2one('dds_camp.activity.period', 'Period'),
        'staff_ids': fields.many2many('dds_camp.event.participant',
                                      'dds_camp_activity_staff_rel',
                                      'act_ins_id', 'par_id', 'Staff'),
        'ticket_ids': fields.one2many('dds_camp.activity.ticket', 'act_ins_id',
                                      'Tickets'),
        'actins_date_begin': fields.related('period_id', 'date_begin',
                                            type='datetime',
                                            string='Start Date/Time',
                                            store=True),
        'ag06_08': fields.function(_calc_agegroup, type='integer',
                                   string='Age 6-8', method=True, multi='AGE'),
        'ag09_10': fields.function(_calc_agegroup, type='integer',
                                   string='Age 9-10',
                                   method=True, multi='AGE'),
        'ag11_12': fields.function(_calc_agegroup, type='integer',
                                   string='Age 11-12', method=True,
                                   multi='AGE'),
        'ag13_16': fields.function(_calc_agegroup, type='integer',
                                   string='Age 13-16', method=True,
                                   multi='AGE'),
        'ag17_22': fields.function(_calc_agegroup, type='integer',
                                   string='Age 17-22', method=True,
                                   multi='AGE'),
        'ag22p': fields.function(_calc_agegroup, type='integer',
                                 string='Age 22+', method=True, multi='AGE'),
    }
...
class dds_camp_activity_ticket(osv.osv):
    ...
    _columns = {
        'name': fields.char('Own Note', size=128,
                            help='You can add a Note for own use. It will be shown on activity list etc. I will NOT be read/answered by the Staff.'),
    ...
    def run_scheduler(self, cr, uid, automatic=False, use_new_cursor=False,
        ...
        ) - datetime.timedelta(minutes=15)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        ...
        exp_time = (datetime.datetime.now(
        ) - datetime.timedelta(minutes=240)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
